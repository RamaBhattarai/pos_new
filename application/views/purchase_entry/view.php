            ...
            <div id="invoice-template" class="card-body">
                <div class="row wrapper white-bg page-heading">
                    ...
                    <div class="col-lg-12">
                        ...
                        if ($invoice['status'] != 'canceled') { ?>
                            ...
                            <a href="<?php echo 'edit_entry?id=' . $invoice['iid']; ?>" class="btn btn-warning"><i
                                        class="fa fa-pencil"></i> <?php echo $this->lang->line('Edit Entry') ?> </a>
                            ...
                            <div class="btn-group ">
                                <button type="button" class="btn btn-success btn-min-width dropdown-toggle"
                                        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i
                                            class="fa fa-print"></i> <?php echo $this->lang->line('Print Entry') ?>
                                </button>
                                <div class="dropdown-menu">
                                    <a class="dropdown-item"
                                       href="<?= base_url('purchase/printinvoice?id=' . $invoice['iid'] . '&type=entry'); ?>"><?php echo $this->lang->line('Print') ?></a>
                                    <div class="dropdown-divider"></div>
                                    <a class="dropdown-item"
                                       href="<?= base_url('purchase/printinvoice?id=' . $invoice['iid'] . '&type=entry&d=1'); ?>"><?php echo $this->lang->line('PDF Download') ?></a>
                ...
                <div id="invoice-company-details" class="row mt-2">
                    ...
                    <div class="col-md-6 col-sm-12 text-xs-center text-md-right">
                        <h2><?php echo $this->lang->line('Purchase Entry') ?></h2>
                        <p class="pb-1"> <?php echo prefix(2) . $invoice['tid'] ?></p>
                        <p class="pb-1"><?php echo $this->lang->line('Reference') . ':' . $invoice['refer'] ?></p>
                        ...
                        <ul class="px-0 list-unstyled">
                            ...
                            <li class="text-bold-800"><a
                                        href="<?php echo base_url('supplier/view?id=' . $invoice['cid']) ?>"><strong
                                            class="invoice_a"><?php echo $invoice['name'] ?></strong></a></li>
                            <li><?php echo $invoice['address'] ?></li>
                            <li><?php echo $invoice['city'] . ',' . $invoice['country'] ?></li>
                            <li><?php echo $this->lang->line('Phone') . ': ' . $invoice['phone'] ?></li>
                            <li><?php echo $this->lang->line('Email') . ': ' . $invoice['email'] ?></li>
                            </li>
                        </ul>

                    </div>
                    <!-- <div class="offset-md-3 col-md-3 col-sm-12 text-xs-center text-md-left">
                        <?php echo '<p><span class="text-muted">' . $this->lang->line('Entry Date') . ' :</span> ' . dateformat($invoice['invoicedate']) . '</p> <p><span class="text-muted">' . $this->lang->line('Due Date') . ' :</span> ' . dateformat($invoice['invoiceduedate']) . '</p>  <p><span class="text-muted">' . $this->lang->line('Terms') . ' :</span> ' . $invoice['termtit'] . '</p>';
                        ?>
                    </div> -->

                    <div class="offset-md-3 col-md-3 col-sm-12 text-xs-center text-md-left">
    <p>
        <span class="text-muted"><?php echo $this->lang->line('Entry Date'); ?> :</span> 
                        ...
                        <div class="table-responsive col-sm-12">
                            <table class="table table-striped">
                                ...
                                foreach ($products as $row) {
                                    ...
                                    $gst = !empty($row['totaltax']) ? $row['totaltax'] / 2 : 0;
                                    $rate = !empty($row['tax']) ? $row['tax'] / 2 : 0;
                                    echo '<tr>
                            <th scope="row">' . $c . '</th>
                            <td>' . $row['product'] . '</td> 
                            <td>' . (!empty($row['code']) ? $row['code'] : '-') . '</td>
                            <td>' . amountExchange($row['price'], 0, $this->aauth->get_user()->loc) . '</td>
                             <td>' . amountFormat_general($row['qty']) . (!empty($row['unit']) ? $row['unit'] : '') . '</td>
                              <td>' . amountExchange((!empty($row['totaldiscount']) ? $row['totaldiscount'] : 0), 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s((!empty($row['discount']) ? $row['discount'] : 0)) . $this->lang->line($invoice['format_discount']) . ')</td>
                            <td>' . amountExchange($gst, 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($rate) . '%)</td>
                             <td>' . amountExchange($gst, 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($rate) . '%)</td>                           
                            <td>' . amountExchange($row['subtotal'] - (!empty($row['totaldiscount']) ? $row['totaldiscount'] : 0), 0, $this->aauth->get_user()->loc) . '</td>
                        </tr>';

                                    echo '<tr><td colspan=9>' . (!empty($row['product_des']) ? $row['product_des'] : '') . '</td></tr>';
                                    $c++;
                                } ?>
...
<th scope="row">' . $c . '</th>
                            ...
                            <td>' . amountExchange($row['price'], 0, $this->aauth->get_user()->loc) . '</td>
                             <td>' . amountFormat_general($row['qty']) . (!empty($row['unit']) ? $row['unit'] : '') . '</td>
                              <td>' . amountExchange((!empty($row['totaldiscount']) ? $row['totaldiscount'] : 0), 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s((!empty($row['discount']) ? $row['discount'] : 0)) . $this->lang->line($invoice['format_discount']) . ')</td>
                            <td>' . amountExchange((!empty($row['totaltax']) ? $row['totaltax'] : 0), 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s((!empty($row['tax']) ? $row['tax'] : 0)) . '%)</td>
                                            
                            <td>' . amountExchange($row['subtotal'] - (!empty($row['totaldiscount']) ? $row['totaldiscount'] : 0), 0, $this->aauth->get_user()->loc) . '</td>
                        </tr>';

                                        echo '<tr><td colspan=8>' . (!empty($row['product_des']) ? $row['product_des'] : '') . '</td></tr>';
                                        $c++;
                                    } ?>
...
<td>' . amountExchange($row['subtotal'] - (!empty($row['totaldiscount']) ? $row['totaldiscount'] : 0), 0, $this->aauth->get_user()->loc) . '</td>
    ...
    echo '<tr><td colspan="7">' . (!empty($row['product_des']) ? $row['product_des'] : '') . '</td></tr>';
    $c++;
}
?>
                    ...
                    <div class="row">
                        <div class="col mb-1"><label
                            ...
                            <input type="text" class="form-control"
                                   ...
                                   value="Payment for purchase entry #<?php echo $invoice['tid'] ?>"></div>
                    </div>
...
<div id="cancel_bill" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title"><?php echo $this->lang->line('Cancel Purchase Entry') ?></h4>
            </div>
            <div class="modal-body">
