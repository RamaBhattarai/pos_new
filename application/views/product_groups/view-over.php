...
<?php if ($product_variations) {
    ...
    <table class="table table-striped table-bordered">
        ...
        foreach ($product_variations as $product_variation) {
            $variant_display = isset($product_variation['variant_name']) ? $product_variation['variant_name'] : 'N/A';
            
            echo '<tr>
                    <td>
                        <a href="' . base_url() . 'products/edit?id=' . $product_variation['pid'] . '" class="btn btn-primary btn-sm">
                            <span class="icon-pencil"></span> ' . $this->lang->line('Edit') . '
                        </a>
                    </td>
                    <td>' . $variant_display . '</td>
                    <td>' . $product_variation['product_name'] . '</td>
                    <td>' . $this->lang->line('Stock') . ': ' . $product_variation['qty'] . '</td>
                    <td>Price: ' . (isset($product_variation['product_price']) ? amountExchange($product_variation['product_price'], 0, $this->aauth->get_user()->loc) : 'N/A') . '</td>
                  </tr>';
        } ?>
        </tbody>
    </table>
