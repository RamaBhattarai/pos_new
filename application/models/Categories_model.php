...
FROM pos_product_cat WHERE c_type='$type' AND rel_id='$rel'
...
ORDER BY id DESC");
    ...
    public function warehouse_list()
    {
        ...
        if ($this->aauth->get_user()->loc) {
            ...
            if (BDATA) $where = "WHERE  (loc=" . $this->aauth->get_user()->loc . " OR pos_warehouse.loc=0) ";
        }


        $query = $this->db->query("SELECT id,title
FROM pos_warehouse $where 
...
ORDER BY id DESC");
    ...
    {
        $whr = '';
        if (!BDATA) $whr = "WHERE  (pos_warehouse.loc=0) ";
        if ($this->aauth->get_user()->loc) {
            $whr = "WHERE  (pos_warehouse.loc=" . $this->aauth->get_user()->loc . " ) ";
            if (BDATA) $whr = "WHERE  (pos_warehouse.loc=" . $this->aauth->get_user()->loc . " OR pos_warehouse.loc=0) ";
        }

        $query = $this->db->query("SELECT c.*,p.pc,p.salessum,p.worthsum,p.qty FROM pos_product_cat AS c LEFT JOIN ( SELECT pos_products.pcat,COUNT(pos_products.pid) AS pc,SUM(pos_products.product_price*pos_products.qty) AS salessum, SUM(pos_products.fproduct_price*pos_products.qty) AS worthsum,SUM(pos_products.qty) AS qty FROM pos_products LEFT JOIN pos_warehouse ON pos_products.warehouse=pos_warehouse.id  $whr GROUP BY pos_products.pcat ) AS p ON c.id=p.pcat WHERE c.c_type=0");
        return $query->result_array();
    }
    ...
    public function category_sub_stock($id = 0)
    {
        ...
        $query = $this->db->query("SELECT c.*,p.pc,p.salessum,p.worthsum,p.qty,p.sub_id FROM pos_product_cat AS c LEFT JOIN ( SELECT pos_products.sub_id,COUNT(pos_products.pid) AS pc,SUM(pos_products.product_price*pos_products.qty) AS salessum, SUM(pos_products.fproduct_price*pos_products.qty) AS worthsum,SUM(pos_products.qty) AS qty FROM pos_products LEFT JOIN pos_warehouse ON pos_products.warehouse=pos_warehouse.id  $whr GROUP BY pos_products.sub_id ) AS p ON c.id=p.sub_id WHERE c.c_type=1 AND c.rel_id='$id'");
        return $query->result_array();
    }
    ...
    public function warehouse()
    {
        ...
        $query = $this->db->query("SELECT c.*,p.pc,p.salessum,p.worthsum,p.qty FROM pos_warehouse AS c LEFT JOIN ( SELECT warehouse,COUNT(pid) AS pc,SUM(product_price*qty) AS salessum, SUM(fproduct_price*qty) AS worthsum,SUM(qty) AS qty FROM  pos_products GROUP BY warehouse ) AS p ON c.id=p.warehouse  $where");
        return $query->result_array();
    }
    ...
    public function cat_ware($id, $loc = 0)
    {
        ...
        $query = $this->db->query("SELECT c.id AS cid, w.id AS wid,c.title AS catt,w.title AS watt FROM pos_products AS p LEFT JOIN pos_product_cat AS c ON p.pcat=c.id LEFT JOIN pos_warehouse AS w ON p.warehouse=w.id WHERE
p.pid='$id' $qj ");
    ...
    public function addnew($cat_name, $cat_desc, $cat_type = 0, $cat_rel = 0)
    {
        ...
        if ($this->db->insert('pos_product_cat', $data)) {
...
if ($count >= $warehouse_limit) {
    ...
    return;
        ...
        if ($this->db->insert('pos_warehouse', $data)) {
    ...
    public function edit($catid, $product_cat_name, $product_cat_desc, $cat_type, $cat_rel, $old_cat_type)
    {
        ...
        if ($this->db->update('pos_product_cat')) {
    ...
    public function editwarehouse($catid, $product_cat_name, $product_cat_desc, $lid)
    {
        ...
        if ($this->db->update('pos_warehouse')) {
