...
class Registerlog
{
    ...
    {
        $this->RI->db->from('pos_register');
        $this->RI->db->where('pos_register.id', $id);
              $this->RI->db->join('pos_users', 'pos_register.uid=pos_users.id', 'left');
            if ($this->RI->aauth->get_user()->loc) {
            $this->RI->db->group_start();
            $this->RI->db->where('pos_users.loc', $this->RI->aauth->get_user()->loc);
            if (BDATA) $this->RI->db->or_where('pos_users.loc', 0);
            $this->RI->db->group_end();
        } elseif (!BDATA) {
            $this->RI->db->where('pos_users.loc', 0);
        }
        $query = $this->RI->db->get();
        $result = $query->row_array();
        if ($result) {
    ...
    public function create($id, $cash, $card, $bank, $cheque)
    {
        ...
        return $this->RI->db->insert('pos_register', $data);
    }
    ...
    public function update($id, $cash = 0,  $card = 0, $bank = 0, $cheque = 0,$change = 0)
    {
        ...
        $this->RI->db->where('uid', $id);
        $this->RI->db->where('active', 1);
        $this->RI->db->update('pos_register');
    }
    ...
    {
       $this->RI->db->select('pos_register.*,pos_users.username,pos_users.loc');
        $this->RI->db->from('pos_register');
       $this->RI->db->join('pos_users','pos_register.uid=pos_users.id','left');
       if($loc)    $this->RI->db->where('pos_users.loc',$loc);
        $query = $this->RI->db->get();
